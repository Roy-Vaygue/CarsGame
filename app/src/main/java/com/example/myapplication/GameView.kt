    ...
    override fun onDraw(canvas: Canvas?){
        ...
        for (i in otherCars.indices){
            try {
                ...
                var d2 = resources.getDrawable(R.drawable.obstacle, null)

                d2.setBounds(
                    carX + 25 , carY - carHeight , carX + carWidth - 25 , carY
                )
                d2.draw(canvas)
                if (otherCars[i]["lane"] as Int == myCarPosition && otherCars[i]["collisionHappen"] == false){
                    if (carY > viewHeight - 2 - carHeight && carY < viewHeight -2){
                        lives--
                        otherCars[i]["collisionHappen"] = true
        ...
        canvas.drawText("Lives : $lives" , 600f , 80f, myPaint!!)
        invalidate()
    }
